...
lists3=strhtml.select('[class="title-sub"]')

df = pandas.DataFrame()
i=0
for fr in lists:
    context = fr.string+ '：'+ lists2[i].string.replace('¥','') + ';点赞：' + lists3[i].string.replace('条好评','').replace('\n','').strip() 
    print(context)
    i=i+1
    tmp = pandas.DataFrame([context.split('：')], columns=["书名", "价格","点赞"])
    df = df.append(tmp, ignore_index=True)

#print(df)
df.to_csv('book.csv', encoding='gbk', index=False)
